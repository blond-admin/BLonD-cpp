...
private:
	...
	void kick(const uint index) {
		...
		kick(Beam->dt.data(), Beam->dE.data(), RfP->n_rf, RfP->voltage, RfP->omega_RF, RfP->phi_RF,
			Beam->n_macroparticles, acceleration_kick[index], index);
...
#pragma omp parallel for
			for (int i = 0; i < n_macroparticles; ++i) {
				...
				beam_dE[i] +=
					current_voltage *
					mymath::fast_sin(current_omega_RF * beam_dt[i] + current_phi) + acc_kick;
			}
		}
	}
