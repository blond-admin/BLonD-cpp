/*
OpenCL performance implies that data stays in the device memory as long as
possible.
Thus all algorithm-requiered data shall be provided to an external user only on
demand. That implies
protection of all vectors such as beam_dE (a user needs to ask for beam_dE and
tall us if he changed it,
...
We warm up all Device functions in constructor (text->binary->initiated, ready
to run kernel)
We do not use BOOST_COMPUTE_CLOSURE+transform because:
a) BOOST_COMPUTE_STRINGIZE_SOURCE allows __constant/__local/read_only explicit
setup
b) we can be sure when the Device functions are compiled (and that they are
compiled only once)
Developers note: printf works in OpenCL kernels, reading OpenCL error codes is
useful (see https://streamcomputing.eu/blog/2013-04-28/opencl-error-codes/)
*/
...
struct TC1_OpenCL {
    TC1_OpenCL() {
        ...
        const std::string source_kick =
            BOOST_COMPUTE_STRINGIZE_SOURCE(__kernel void kick(
                __global double* beam_dt, __global double* beam_dE,
                double current_voltage, double current_omega_RF,
                double current_phi, double acc_kick_gpu) {
                int id = get_global_id(0);
                beam_dE[id] +=
                    acc_kick_gpu +
                    current_voltage *
                        sin(current_omega_RF * beam_dt[id] + current_phi);
            });

        const std::string source_drift = BOOST_COMPUTE_STRINGIZE_SOURCE(
            __kernel void drift(__global double* beam_dE,
                                __global double* beam_dt, double T_x_coeff) {
                int id = get_global_id(0);
                beam_dt[id] += T_x_coeff * beam_dE[id];
            });

        const std::string source_histogram =
            BOOST_COMPUTE_STRINGIZE_SOURCE(__kernel void histogram(
                __global double* input, __global int* output, int cut_left,
                int cut_right) {
                int id = get_global_id(0);
                int key = input[id];
        ...
        queue = command_queue(context, gpu);
  ...
  private:
    ...
    // Host implementation
    void kick(const int n_rf, const f_vector_2d_t& __restrict voltage,
              const f_vector_2d_t& __restrict omega_RF,
              const f_vector_2d_t& __restrict phi_RF, const uint index) {
        using namespace boost::compute;
        ...
        for (int j = 0; j < n_rf; ++j) {
            ...
            kick(dt, dE, current_voltage, current_omega_RF, current_phi,
                 acc_kick_gpu);
        }
    }
    ...
    inline void drift(const uint index) {
        ...
        drift(GP->t_rev[index], RfP->length_ratio, RfP->eta_0(index),
              RfP->beta(index), RfP->energy(index));
    }

    // Host implementation
    inline void drift(const ftype T0, const ftype length_ratio,
                      const ftype eta_zero, const ftype beta,
                      const ftype energy) {
    ...
    void kick(const boost::compute::vector<ftype>& beam_dt,
              boost::compute::vector<ftype>& beam_dE, double current_voltage,
              double current_omega_RF, double current_phi,
              double acc_kick_gpu) {
    ...
    void drift(const boost::compute::vector<ftype>& beam_dE,
               boost::compute::vector<ftype>& beam_dt, double T_x_coeff) {
    ...
    void histogram(const boost::compute::vector<ftype>& input,
                   boost::compute::vector<int>& output, int cut_left,
                   int cut_right) {
        using namespace boost::compute;
    ...
    void build_kernel(const std::string& function_name,
                      const std::string& source,
                      boost::compute::kernel& kernel) {
        using namespace boost::compute;
        program program = program::create_with_source(source, context);
        program.build();
    ...
    template <class T1, class T2> void move_array(const T1& from, T2& to) {
        to.resize(from.size());
    ...
    void move_2d_from_host(const T1& from, T2& to) {
        ...
        for (auto& row : from) {
            boost::compute::copy(row.begin(), row.end(),
                                 to.begin() + counter * rowLength, queue);
            ++counter;
        }
    }

    template <class T1, class T2> void move_2d_to_host(const T1& from, T2& to) {
        int rowLength = to.begin()->size();
        for (auto i = 0; i < from.size(); ++i) {
            boost::compute::copy(from.begin() + i * rowLength,
                                 from.end() + (i + 1) * rowLength,
                                 (to.begin() + i)->begin(), queue);
        }
    }
};
#endif