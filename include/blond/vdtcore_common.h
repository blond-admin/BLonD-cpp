...
namespace blond {
    namespace vdt {
        ...
        namespace details {
            ...
            /// Converts a double to an unsigned long long long
            ...
            //------------------------------------------------------------------------------
            /// Makes an AND of a double and a unsigned long long long
            inline double dpANDuint64(const double x, const uint64_t i)
            {
                return uint642dp(dp2uint64(x) & i);
            }
            //------------------------------------------------------------------------------
            /// Makes an OR of a double and a unsigned long long long
            inline double dpORuint64(const double x, const uint64_t i)
            {
                return uint642dp(dp2uint64(x) | i);
            }

            /// Makes a XOR of a double and a unsigned long long long
            ...
            inline double getMantExponent(const double x, double &fe)
            {
                ...
                // chop the head of the number: an int contains more than 11 bits
                // (32)
                int32_t e =
             ...
             * These functions do not distinguish between -0.0 and 0.0, so are not
             *IEC6509
             * compliant for argument -0.0
            **/
            inline double fpfloor(const double x)
            {
                // no problem since exp is defined between -708 and 708. Int is
                // enough for it!
