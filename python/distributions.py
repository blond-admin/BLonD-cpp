...
def matched_from_line_density(beam_beta, beam_energy, beam_charge,
    ...
    *Function to generate a beam by inputing the line density. The distribution
    ...
    eom_factor_dE = abs(slippage_factor) / (2*beam_beta**2. * beam_energy)
    eom_factor_potential = np.sign(
        slippage_factor) * beam_charge / t_rev_0
    ...
    if extraVoltageDict != None:
        ...
        extra_potential_input = - eom_factor_potential * \
            np.insert(cumtrapz(extra_voltage_input, dx=extra_voltage_time_input[
                      1] - extra_voltage_time_input[0]), 0, 0)
    ...
    if line_density_options['type'] != 'user_input':
        ...
        line_density = line_density_function(time_line_den, line_density_options['type'],
                                             line_density_options[
                                                 'bunch_length'],
                                             exponent=line_density_options[
                                                 'exponent'],
                                             bunch_position=(time_coord_array[0]+time_coord_array[-1])/2)

        line_density = line_density - np.min(line_density)
        line_density = line_density / \
            np.sum(line_density) * beam_n_macroparticles

    elif line_density_options['type'] == 'user_input':
        # Time coordinates for the line density
    ...
    for i in range(0, n_iterations):
        ...
        # present)
        # NOTE TotalInducedVoltage has been removed for now
    ...
    for abel_index in range(0, abel_both_step):
        ...
        if (half_option == 'first') or (half_option == 'both' and abel_index == 0):
        ...
        if (half_option == 'second') or (half_option == 'both' and abel_index == 1):
            for i in range(0, n_points_abel):
                integrand = line_den_diff_abel[
    ...
    indexes = np.random.choice(
        range(0, np.size(density_grid)), beam_n_macroparticles, p=density_grid.flatten())

    # NOTE the code has been slightly altered by kiliakis
    ...
    for i in range(len(beam_dt)):
        beam_dt[i] = temp[i]+(rand_nums[i] - 0.5) * \
            (time_coord_for_grid[1]-time_coord_for_grid[0])

    temp = deltaE_grid.flatten()[indexes]
    rand_nums = np.random.rand(beam_n_macroparticles)
    for i in range(len(beam_dE)):
        beam_dE[i] = temp[i] + \
            (rand_nums[i] - 0.5) * (deltaE_for_grid[1]-deltaE_for_grid[0])
...
def matched_from_distribution_density(beam_beta, beam_energy, beam_charge,
                                      beam_n_macroparticles, beam_dt, beam_dE,
                                      eta_0, t_rev_0,
    ...
    *Function to generate a beam by inputing the distribution density (by
    ...
    for i in range(0, n_iterations):
        ...
        grid_indexes = np.arange(
            0, n_points_grid) * n_points_potential / n_points_grid
        time_coord_low_res = np.interp(
            grid_indexes, potential_well_indexes, time_coord_sep)
        deltaE_coord_array = np.linspace(-
                                         max_deltaE, max_deltaE, n_points_grid)
        potential_well_low_res = np.interp(
            grid_indexes, potential_well_indexes, potential_well_sep)
        time_grid, deltaE_grid = np.meshgrid(
            time_coord_low_res, deltaE_coord_array)
        potential_well_grid = np.meshgrid(
            potential_well_low_res, potential_well_low_res)[0]
        ...
        for i in range(0, n_points_grid):
            dE_trajectory = np.sqrt(
                (potential_well_low_res[i] - H_array_dE0)/eom_factor_dE)
            dE_trajectory[np.isnan(dE_trajectory)] = 0
            J_array_dE0[i] = 2 / \
                (2*np.pi) * np.trapz(dE_trajectory, dx=time_resolution)

        warnings.filterwarnings("default")

        # Sorting the H and J functions in order to be able to interpolate the
        # function J(H)
        ...
        if distribution_options.has_key('bunch_length'):
            value = distribution_options['bunch_length']
            distribution_options['bunch_length'] = float(value)
            tau = 0.0
            # Choice of either H or J as the variable used
            if density_variable_option == 'density_from_J':
            ...
            elif density_variable_option == 'density_from_H':
                ...
                raise RuntimeError(
                    'The density_variable option was not recognized')

            bunch_length_accuracy = (
                time_coord_low_res[1] - time_coord_low_res[0]) / 2.
            ...
            while np.abs(distribution_options['bunch_length']-tau) > bunch_length_accuracy:
                ...
                if density_variable_option == 'density_from_J':
                    density_grid = distribution_density_function(
                        J_grid, distribution_options['type'], X0, distribution_options['exponent'])
                elif density_variable_option == 'density_from_H':
                    density_grid = distribution_density_function(
                        H_grid, distribution_options['type'], X0, distribution_options['exponent'])
                ...
                if (line_density > 0).any():
                    tau = 4.0 * np.sqrt(np.sum((time_coord_low_res - np.sum(line_density * time_coord_low_res) / np.sum(
                        line_density))**2 * line_density) / np.sum(line_density))
                    ...
                    #     if distribution_options['bunch_length_fit'] == 'gauss':
                    #         slices.bl_gauss = tau
                    #         slices.bp_gauss = np.sum(line_density * time_coord_low_res) / np.sum(line_density)
                    #         slices.gaussian_fit()
                    #         tau = slices.bl_gauss
                    #     elif distribution_options['bunch_length_fit'] == 'fwhm':
                    #         slices.fwhm()
                    #         tau = slices.bl_fwhm
                    #     elif distribution_options['bunch_length_fit'] == 'end_to_end':
        ...
        if distribution_options['type'] != 'user_input_table':
            if density_variable_option == 'density_from_J':
                ...
                density_grid = distribution_density_function(
                    J_grid, distribution_options['type'], J0, distribution_options['exponent'])
            elif density_variable_option == 'density_from_H':
                if distribution_options.has_key('emittance'):
                    H0 = np.interp(
                        distribution_options['emittance'] / (2*np.pi), sorted_J_dE0, sorted_H_dE0)
                density_grid = distribution_density_function(
                    H_grid, distribution_options['type'], H0, distribution_options['exponent'])
        else:
            if density_variable_option == 'density_from_J':
                density_grid = np.interp(J_grid, distribution_options[
                                         'user_table_action'], distribution_options['user_table_density'])
            elif density_variable_option == 'density_from_H':
                density_grid = np.interp(H_grid, distribution_options[
                                         'user_table_action'], distribution_options['user_table_density'])
        ...
        # Induced voltage contribution
        # TODO Removed TotalInducedVoltage
        # if TotalInducedVoltage is not None:
        if False:
            break
            # # Calculating the line density
            # line_density = np.sum(density_grid, axis = 0)
            # line_density = line_density / np.sum(line_density) * beam_n_macroparticles
    ...
    # beam_dt = np.ascontiguousarray(time_grid.flatten()[indexes]+(np.random.rand(beam_n_macroparticles) -0.5)*(time_coord_low_res[1]-time_coord_low_res[0]))
    # beam_dE = np.ascontiguousarray(deltaE_grid.flatten()[indexes] + (np.random.rand(beam_n_macroparticles) - 0.5) * (deltaE_coord_array[1]-deltaE_coord_array[0]))
    ...
    rand_nums = np.random.rand(beam_n_macroparticles)
    for i in range(len(beam_dE)):
        beam_dE[i] = temp[i] + (rand_nums[i] - 0.5) * \
            (deltaE_coord_array[1]-deltaE_coord_array[0])
...
def line_density_function(coord_array, dist_type, bunch_length, bunch_position=0, exponent=None):
    ...
    *Line density*
    ...
    if dist_type in ['binomial', 'waterbag', 'parabolic_amplitude', 'parabolic_line']:
        ...
        warnings.filterwarnings("ignore")
        density_function = (
            1 - ((coord_array - bunch_position) / (bunch_length/2))**2)**(exponent+0.5)
        warnings.filterwarnings("default")
        density_function[
        ...
        density_function = np.exp(- (coord_array -
                                     bunch_position)**2 / (2*sigma**2))
        return density_function
    elif dist_type == 'cosine_squared':
        warnings.filterwarnings("ignore")
        density_function = np.cos(
            np.pi * (coord_array - bunch_position) / bunch_length)**2
        warnings.filterwarnings("default")
        density_function[
            np.abs(coord_array - bunch_position) > bunch_length/2] = 0
        return density_function
