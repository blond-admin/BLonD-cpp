...
TEST(IRFFT_with_mem, rfft_even)
{
   ...
   fft::destroy_plans(planVec);

}

TEST(IRFFT_with_mem, rfft_odd)
{
   //std::string params = "../unit-tests/references/MyMath/fft/";
   ...
   fft::irfft(out1, out4, planVec, 15);
   //util::dump(out4.data(), out4.size(), "ifft after rfft\n");
...
TEST(IRFFT_with_mem, irfft_big)
{
   ...
   fft::rfft(in1,  in2, planVec,  in1.size());

   fft::irfft(in2, out, planVec);
...
TEST(IRFFT_with_mem, irfft_big2)
{
...
TEST(IRFFT_with_mem, irfft_test)
{
...
TEST(RFFT_with_mem, irfft)
{
   ...
   fft::fft(az, az, planVec, 512);
   //printf("ok here\n");

   fft::fft(bz, bz, planVec, 512);
