...
if (i < 100)
in[i] =
complex_t(std::sin(i), std::sqrt(i)
);
else
in[i] =
complex_t(std::sin(i),
...
begin(), std::multiplies<complex_t>()
...
ftype ref = res[i];
ftype real2 = real[i];
ASSERT_NEAR(ref, real2, epsilon
*
std::max(fabs(ref), fabs(real2)
))
