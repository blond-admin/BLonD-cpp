...
TEST_F(testPSBPhaseNoiseInjection, generate_lin1)
{
   ...
   auto psbNoise = new PSBPhaseNoiseInjection(2, 10, 0.8,
         1.2, 2e-6, 12, 75, PhaseNoise::predistortion_t::linear);
   psbNoise->generate();
   ...
   // ASSERT_NEAR(meanV, meanR, maxV * epsilon * std::min(fabs(meanV), fabs(meanR)));
